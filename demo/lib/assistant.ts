...
export interface ToolCallItem {
	type: "tool_call";
	tool_type: "file_search_call" | "web_search_call" | "function_call" | "mcp_call" | "code_interpreter_call";
...
export const processMessages = async () => {
	const { chatMessages, conversationItems, setChatMessages, setConversationItems, setAssistantLoading } =
	...
	await handleTurn(allConversationItems, tools, async ({ event, data }) => {
		switch (event) {
			...
			case "response.output_text.annotation.added": {
				...
				if (
					...
					if (contentItem && contentItem.type === "output_text") {
						...
						if (annotation) {
							contentItem.annotations = [...(contentItem.annotations ?? []), normalizeAnnotation(annotation)];
						}
					}
				}
			...
			case "response.output_item.done": {
				...
				if (toolCallMessage && toolCallMessage.type === "tool_call" && toolCallMessage.tool_type === "mcp_call") {
			...
			case "response.code_interpreter_call.completed": {
				const { item_id } = data;
				const toolCallMessage = chatMessages.find((m) => m.type === "tool_call" && m.id === item_id) as
					| ToolCallItem
					| undefined;
				if (toolCallMessage) {
					toolCallMessage.status = "completed";
